...
export class ListNotasComponent implements OnInit {
  ...
  displayedColumns: string[] = ["dataEmissao", "valorTotal", "valorDesconto", "nota", "local"];
  ...
  seleciona(){
    let ano: number  =  Number(moment(this.date.value).format('YYYY'));
    let mes: number  = Number(moment(this.date.value).format('MM'));    
    this.serCtrl.listaNotasMes(ano,mes).subscribe((its) => {
  ...
  click(){
    console.log(moment(this.date.value).format('YYYY'));
    console.log(moment(this.date.value).format('MM'));
